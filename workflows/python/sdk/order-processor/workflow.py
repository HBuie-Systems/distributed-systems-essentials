...
def order_processing_workflow(ctx: DaprWorkflowContext, order_payload_str: str):
    ...
    if order_payload["total_cost"] > 5000:
        ...
        if approval_result == False:  # noqa: E712
            yield ctx.call_activity(
                notify_activity,
                input=Notification(message=f"Order {order_id} was not approved"),
            )
            return OrderResult(processed=False)
    ...
    yield ctx.call_activity(
        notify_activity, input=Notification(message=f"Order {order_id} has completed!")
    )
...
def process_payment_activity(ctx: WorkflowActivityContext, input: PaymentRequest):
    ...
    print(
        f"Payment for request ID {input.request_id} processed successfully", flush=True
    )
...
def update_inventory_activity(
    ...
    with DaprClient(
        ...
        if new_quantity < 0:
            raise ValueError(
                "Inventory update for request ID "
