...
def resiliency_and_compensation_workflow(ctx: wf.DaprWorkflowContext, wf_input: int):
    default_retry_policy = wf.RetryPolicy(
        max_number_of_attempts=3, first_retry_interval=timedelta(seconds=2)
    )

    result1 = yield ctx.call_activity(
        minus_one, input=wf_input, retry_policy=default_retry_policy
    )
    wf_result = None

    try:
        wf_result = yield ctx.call_activity(
            division, input=result1, retry_policy=default_retry_policy
        )
