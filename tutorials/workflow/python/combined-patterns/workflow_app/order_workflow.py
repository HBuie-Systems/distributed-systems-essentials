...
def order_workflow(ctx: wf.DaprWorkflowContext, order: Order):
    ...
    tasks = [
        ...
        ctx.call_activity(
            check_shipping_destination, input=order.customer_info.model_dump()
        ),
    ]
    task_results = yield wf.when_all(tasks)  # noqa: E703
    task_results = [
        ActivityResult.model_validate(task_result) for task_result in task_results
    ]
    ...
    shipment_registration_status = ShipmentRegistrationStatus.model_validate(
...
def check_inventory(
    ctx: wf.WorkflowActivityContext, order_item: OrderItem
) -> ActivityResult:
...
def check_shipping_destination(
    ctx: wf.WorkflowActivityContext, customer_info: CustomerInfo
) -> ActivityResult:
...
def reimburse_customer(
    ctx: wf.WorkflowActivityContext, order: Order
) -> ReimburseCustomerResult:
    order = Order.model_validate(order)  # noqa: E703
