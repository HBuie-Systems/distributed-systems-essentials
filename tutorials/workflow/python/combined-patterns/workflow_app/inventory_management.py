...
def create_default_inventory() -> None:
    product_inventory_item = ProductInventoryItem(
        product_id="RBD001", product_name="Rubber Duck", quantity=50
    )
...
def get_inventory_item(product_id: str) -> ProductInventoryItem | None:
    with DaprClient() as dapr_client:
        state_response = dapr_client.get_state(
            store_name=DAPR_INVENTORY_COMPONENT, key=product_id
        )

        if not state_response.data:
            print(f"get_inventory_item: no state response", flush=True)  # noqa: F541
            return None
...
def update_inventory(order_item: OrderItem) -> UpdateInventoryResult:
    ...
    if product_inventory_item is None:
        return UpdateInventoryResult(
            is_success=False,
            message=f"Product not in inventory: {order_item.ProductName}",
        )

    if product_inventory_item.quantity < order_item.quantity:
        return UpdateInventoryResult(
            is_success=False,
            message=f"Inventory not sufficient for: {order_item.ProductName}",
        )
    ...
    return UpdateInventoryResult(
        is_success=True, message=f"Inventory updated for {order_item.product_name}"
    )
